...
let database = [
  {
    ...
    path_to_li: "/", // this will be replaced by a Living Internet OpenGraph tag, <meta property="liog:feed" content="{feed_url}"/>
    status: null,
    description: null,
    thumbnail_url: null
  },
...
document.addEventListener('DOMContentLoaded', async () => {
  for (let i = 0; i < database.length; i++) {
    ...
    if ((!database[i].status) || database[i].status >= 400) {
      ...
      database[i].description = data.description.slice(0, 256); // we don't wanna get too big amir8 ladies
...
async function handleSearch() {
  ...
  for (let i = 0; i < database.length; i += batchSize) {
    ...
    const batchResults = batch.filter((item) => 
      item.url.toUpperCase().includes(upperSearch) || item.description.toUpperCase().includes(upperSearch));
